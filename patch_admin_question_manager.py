...
old_func = """  const handleBulkImport = async () => {
    ...
    try {
      ...
      const importedQs = data.map((row: any, idx) => ({
        ...
        negativeMarks: parseFloat(row['Negative Marks'] || row.negativeMarks) || 0.5,
      ...
      if (importedQs.length === 0) {
        ...
        importedQs.forEach((q, idx) => {
          ...
          const { id, ...qData } = q;
          batch.set(newQRef, { ...qData, order: baseOrder + idx + 1 });
        });
        
        await batch.commit();

        const newTotal = questions.length + importedQs.length;
        const newMarks = questions.reduce((sum, q) => sum + (Number(q.marks) || 0), 0) + importedQs.reduce((sum, q) => sum + (Number(q.marks) || 0), 0);
